...
try:
    DEFAULT_PASSWORD = st.secrets.get("DEFAULT_PASSWORD", "ATC2026")
    ADMIN_RESET_CODE = st.secrets.get("ADMIN_RESET_CODE", "")
except Exception:
...
class Database:
    ...
    def init_tables(self):
        ...
        cursor.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS reset_tokens (
                ...
                reset_by_admin BOOLEAN DEFAULT 0
            );
        ...
        # Insertion des données de démonstration si nécessaire
        ...
        # Configuration du code admin si défini
        if ADMIN_RESET_CODE:
            h, s = hash_password(ADMIN_RESET_CODE)
            cursor.execute(
    ...
    def record_login_attempt(self, user_id, user_type, success, ip_address=None, user_agent=None):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO login_attempts (user_id, user_type, success, ip_address, user_agent)
            VALUES (?, ?, ?, ?, ?)
        """, (user_id, user_type, 1 if success else 0, ip_address, user_agent))
        conn.commit()
        conn.close()

    def get_failed_attempts(self, user_id, user_type, minutes=15):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT COUNT(*) FROM login_attempts
            WHERE user_id = ? AND user_type = ? AND success = 0
            AND attempt_time > datetime('now', '-? minutes')
        """, (user_id, user_type, minutes))
        count = cursor.fetchone()[0]
        conn.close()
        return count

    def is_account_locked(self, user_id, user_type):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT locked_until FROM locked_accounts
            WHERE user_id = ? AND user_type = ? AND unlocked_at IS NULL
        ...
        if row:
            ...
            if datetime.now() < locked_until:
                ...
                self.unlock_account(user_id, user_type, "Déverrouillage automatique")
        return False, None

    def lock_account(self, user_id, user_type, reason=None):
        locked_until = datetime.now() + timedelta(minutes=15)
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO locked_accounts (user_id, user_type, locked_until, reason)
            VALUES (?, ?, ?, ?)
        """, (user_id, user_type, locked_until.strftime("%Y-%m-%d %H:%M:%S"), reason))
        conn.commit()
        conn.close()
        return locked_until

    def unlock_account(self, user_id, user_type, reason=None):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("""
            UPDATE locked_accounts 
            SET unlocked_at = CURRENT_TIMESTAMP, reason = ?
            WHERE user_id = ? AND user_type = ? AND unlocked_at IS NULL
        """, (reason, user_id, user_type))
        conn.commit()
        conn.close()

    def check_login(self, user_id, user_type, password, ip_address=None, user_agent=None):
        # Vérifier le verrouillage
        locked, locked_until = self.is_account_locked(user_id, user_type)
        if locked:
            return False, f"Compte verrouillé jusqu'à {locked_until.strftime('%H:%M')}"
        
        # Vérifier le mot de passe
        if user_type == "eleve":
            result = self.get_eleve_by_id(user_id)
            if not result:
                self.record_login_attempt(user_id, user_type, False, ip_address, user_agent)
                return False, "Utilisateur non trouvé"
            valid = self.verify_password_eleve(user_id, password)
        else:
            result = self.get_instructeur_by_id(user_id)
            if not result:
                self.record_login_attempt(user_id, user_type, False, ip_address, user_agent)
                return False, "Utilisateur non trouvé"
            valid = self.verify_password_instructeur(user_id, password)
        
        if valid:
            self.record_login_attempt(user_id, user_type, True, ip_address, user_agent)
            return True, "OK"
        
        # Échec - vérifier le verrouillage
        self.record_login_attempt(user_id, user_type, False, ip_address, user_agent)
        failed = self.get_failed_attempts(user_id, user_type)
        
        if failed >= 5:
            self.lock_account(user_id, user_type, f"{failed} tentatives échouées")
            return False, f"Compte verrouillé pour 15 minutes (trop de tentatives)"
        
        remaining = 5 - failed
        return False, f"Mot de passe incorrect. {remaining} tentative(s) restante(s)"
    ...
    def delete_all_data(self):
        ...
        for table in ["seances", "groupe_eleves", "groupes", "eleves", "instructeurs",
                      ...
                      "login_attempts", "locked_accounts", "reset_tokens", "reset_attempts", "exports"]:
            cursor.execute(f"DELETE FROM {table}")
        conn.commit()
        conn.close()
...
def radar_login():
    ...
    if role == "eleve":
        ...
        locked, locked_until = db.is_account_locked(eleve_id, "eleve")
        if locked:
            st.error(f"❌ Compte verrouillé jusqu'à {locked_until.strftime('%H:%M')}")
            return
        
        if st.button("🎯 Entrer", type="primary", use_container_width=True):
            success, message = db.check_login(eleve_id, "eleve", password_input)
            if success:
        ...
        with st.expander("🔒 Mot de passe oublié ?"):
            ...
            if st.button("🔄 Réinitialiser mon mot de passe", key="reset_btn_eleve"):
                ...
                db.set_password_eleve(eleve_id, new_temp)
        ...
        locked, locked_until = db.is_account_locked(instr_id, "instructeur")
        if locked:
            st.error(f"❌ Compte verrouillé jusqu'à {locked_until.strftime('%H:%M')}")
            return
        
        if st.button("🎯 Entrer", type="primary", use_container_width=True):
            success, message = db.check_login(instr_id, "instructeur", password_input)
            if success:
        ...
        with st.expander("🔒 Mot de passe oublié ?"):
            ...
            if autres_instructeurs:
                ...
                if st.button("🔄 Réinitialiser (autorisation instructeur)", key="reset_btn_instr"):
                    if db.verify_password_instructeur(autres_instructeurs[autor_selected], autor_password):
                        new_temp = generate_temp_password()
                        db.set_password_instructeur(instr_id, new_temp)
                        st.success(f"✅ Nouveau mot de passe temporaire pour {selected} : **{new_temp}** — à changer via « 🔑 Mon Mot de Passe » dès la connexion.")
                    else:
            ...
            if db.admin_code_configured():
                ...
                st.caption("💡 Configurez le secret Streamlit « ADMIN_RESET_CODE » pour activer une voie de secours ici.")
...
def header_eleve(user):
    ...
    <div style="background:linear-gradient(135deg,#060a12 0%,#0d1a2b 50%,#162a3f 100%);
        ...
        <div style="text-align:right;margin-top:-30px;">
            <span class="cloud-badge">☁️ Streamlit Cloud</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
